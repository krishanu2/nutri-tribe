...
const DEFAULT_MOOD = { bg: 'linear-gradient(135deg, #1a0e0a 0%, #0d0500 100%)', accent: '#f3a213', tagline: 'Bold flavours, honestly made.', glyph: '✦' };
...
function FeaturedProduct({ product }: { product: Product; accent?: string }) {
...
function ProductsContent({ products, categories }: { products: Product[]; categories: string[] }) {
  const searchParams = useSearchParams();
  const initialCategory = categories.includes(searchParams.get('category') || '')
    ? (searchParams.get('category') as string)
    : 'All';
  const [activeCategory, setActiveCategory] = useState(initialCategory);
  const mood = MOODS[activeCategory] || DEFAULT_MOOD;
      ...
      >
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {categories.map((cat) => {
              const m = MOODS[cat] || DEFAULT_MOOD;
...
export default function ProductsPageClient({ products, categories }: { products: Product[]; categories: string[] }) {
  return (
