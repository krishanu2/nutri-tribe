...
import { Loader2, CheckCircle, Sparkles, Wrench, ThumbsUp, Lock } from 'lucide-react';
import TicketStatusBadge from './TicketStatusBadge';

const STATUS_OPTIONS: { value: TicketStatus; label: string; icon: React.ReactNode }[] = [
  { value: 'OPEN',        label: 'Open',        icon: <Sparkles size={14} /> },
  { value: 'IN_PROGRESS', label: 'In Progress', icon: <Wrench size={14} /> },
  { value: 'RESOLVED',    label: 'Resolved',    icon: <ThumbsUp size={14} /> },
  { value: 'CLOSED',      label: 'Closed',      icon: <Lock size={14} /> },
];
...
export default function TicketStatusPanel({ ticketId, currentStatus, adminNote }: Props) {
  ...
  const handleUpdate = async (status?: TicketStatus) => {
    setSaving(true);
    setError('');
    try {
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#7d3627]/8 overflow-hidden">
      <div className="px-6 py-4 border-b border-[#7d3627]/8 flex items-center justify-between">
        <h2 className="font-display font-bold text-lg text-[#7d3627]">Ticket Management</h2>
